...
function cogOrganizationsInfoCtrl($rootScope, $scope, $element, $state, $http, $timeout, loginService, cogOrganizationsService, SweetAlert) {
    ...
    var updateUser = function () {
        $element.find('.update-user').off().on('click', function () {
            ...
            $state.go('cog-admin.developers-update', {
                page: 'manage',
    ...
    var restoreUser = function () {
        $element.find('.restore-user').off().on('click', function () {
            var elem = $(this);
            var userID = elem.attr('data-id');
            restoreProcess(elem, './cogworks/developers/activate', {
                id: userID
            }, 'User', 'restore-user', 'remove-user', function () {
                removeUser()
            });
        });
    };
    var removeUser = function () {
        $element.find('.remove-user').off().on('click', function () {
            var elem = $(this);
            var userID = elem.attr('data-id');
            removeProcess(elem, './cogworks/developers/deactivate', {
                id: userID
            }, 'User', 'remove-user', 'restore-user', function () {
                restoreUser()
            });
        });
    };
    ...
    var restoreProject = function () {
        $element.find('.restore-project').off().on('click', function () {
            var elem = $(this);
            var userID = elem.attr('data-id');
            restoreProcess(elem, './cogworks/projects/activate', {
                id: userID
            }, 'Project', 'restore-project', 'remove-project', function () {
                removeProject()
            });
        });
    };
    var removeProject = function () {
        $element.find('.remove-project').off().on('click', function () {
            ...
            if (files > 0) {
                ...
                removeProcess(elem, './cogworks/projects/deactivate', {
                    id: userID
                }, 'Project', 'remove-project', 'restore-project', function () {
                    restoreProject()
                });
            }
        });
    ...
    var restoreCogfile = function () {
        $element.find('.restore-cogfile').off().on('click', function () {
            var elem = $(this);
            var userID = elem.attr('data-id');
            restoreProcess(elem, './cogworks/cog-files/activate', {
                id: userID
            }, 'Cogfile', 'restore-cogfile', 'remove-cogfile', function () {
                removeCogfile()
            });
        });
    };
    var removeCogfile = function () {
        $element.find('.remove-cogfile').off().on('click', function () {
            var elem = $(this);
            var userID = elem.attr('data-id');
            removeProcess(elem, './cogworks/cog-files/deactivate', {
                id: userID
            }, 'Cogfile', 'remove-cogfile', 'restore-cogfile', function () {
                restoreCogfile()
            });
        });
    };

    cogOrganizationsService.getOrganizationInfo(self, {
        id: orgID
    }, function (data) {
        var str = '';
    ...
    self.addNewUser = function (event) {
        ...
        $state.go('cog-admin.developers-add', {
            page: 'manage',
            orgID: (elem.attr('data-id'))
        });
    };
    self.addNewProject = function (event) {
        var elem = $(event.target);
        $state.go('cog-admin.projects-add', {
            page: 'manage',
            orgID: (elem.attr('data-id'))
        });
    };
    self.addNewCogfile = function (event) {
        var elem = $(event.target);
        $state.go('cog-admin.files-add', {
            page: 'manage',
            org: (elem.attr('data-id'))
        });
    };
}
