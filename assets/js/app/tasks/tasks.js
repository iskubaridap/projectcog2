...
function tasksCtrl($rootScope, $scope, $element, $state, $http, $timeout, loginService, $sce, tasks, $uibModal, SweetAlert) {
    ...
    var changePriority = function (_priority) {
        $http.post("./tasks/update/priority", {
                id: elementID,
                priority: _priority
            })
    ...
    var saveTodo = function () {
        ...
        $http.post("./tasks/todo/update", {
                content: JSON.stringify(self.todoList),
                state: 1,
                id: elementID,
                program: 3
            })
    ...
    var saveInProgress = function () {
        ...
        $http.post("./tasks/todo/update", {
                content: JSON.stringify(self.inProgressList),
                state: 2,
                id: elementID,
                program: 3
            })
    ...
    var saveCompleted = function () {
        ...
        $http.post("./tasks/todo/update", {
                content: JSON.stringify(self.completedList),
                state: 3,
                id: elementID,
                program: 3
            })
                ...
                $scope.ok = function () {
                    ...
                    if ($scope.userIDAry.length > 0 && (($scope.todoTask ? $scope.todoTask : '')).trim().length > 0) {
                        $http.post("./tasks/create", {
                                users: JSON.stringify($scope.userIDAry),
                                content: $scope.todoTask,
                                state: 1,
                                type: 1,
