    ...
    return [function(require, module, exports) {
        ...
        function installColorSpace(colorSpaceName, propertyNames, config) {
            ...
            propertyNames.forEach(function(propertyName, i) {
                prototype[propertyName] = prototype[propertyName === "black" ? "k" : propertyName[0]] = new Function("value", "isDelta", "if (typeof value === 'undefined') {" + "return this._" + propertyName + ";" + "}" + "if (isDelta) {" + "return new this.constructor(" + propertyNames.map(function(otherPropertyName, i) {
        ...
        installColorSpace("XYZ", ["x", "y", "z", "alpha"], {
            fromRgb: function() {
                var convert = function(channel) {
                        return channel > .04045 ? Math.pow((channel + .055) / 1.055, 2.4) : channel / 12.92
                    },
                    r = convert(this._red),
                    g = convert(this._green),
                    b = convert(this._blue);
                return new ONECOLOR.XYZ(r * .4124564 + g * .3575761 + b * .1804375, r * .2126729 + g * .7151522 + b * .072175, r * .0193339 + g * .119192 + b * .9503041, this._alpha)
            },
            rgb: function() {
                var x = this._x,
                    y = this._y,
                    ...
                    convert = function(channel) {
                        return channel > .0031308 ? 1.055 * Math.pow(channel, 1 / 2.4) - .055 : 12.92 * channel
                    };
                return new ONECOLOR.RGB(convert(x * 3.2404542 + y * -1.5371385 + z * -.4985314), convert(x * -.969266 + y * 1.8760108 + z * .041556), convert(x * .0556434 + y * -.2040259 + z * 1.0572252), this._alpha)
            },
            lab: function() {
                var convert = function(channel) {
                        return channel > .008856 ? Math.pow(channel, 1 / 3) : 7.787037 * channel + 4 / 29
                    },
                    x = convert(this._x / 95.047),
                    y = convert(this._y / 100),
        ...
        installColorSpace("LAB", ["l", "a", "b", "alpha"], {
            ...
            xyz: function() {
                var convert = function(channel) {
                        ...
                        return pow > .008856 ? pow : (channel - 16 / 116) / 7.87
                    },
                    y = (this._l + 16) / 116,
                    x = this._a / 500 + y,
                    z = y - this._b / 200;
                return new ONECOLOR.XYZ(convert(x) * 95.047, convert(y) * 100, convert(z) * 108.883, this._alpha)
            }
        });
        installColorSpace("HSV", ["hue", "saturation", "value", "alpha"], {
            ...
            fromRgb: function() {
                var red = this._red,
                    ...
                    delta = max - min,
                    hue, saturation = max === 0 ? 0 : delta / max,
                    value = max;
                if (delta === 0) {
                    hue = 0
                } else {
        ...
        ONECOLOR.installMethod("clearer", function(amount) {
            return this.alpha(isNaN(amount) ? -.1 : -amount, true)
        });
        ONECOLOR.installMethod("darken", function(amount) {
            return this.lightness(isNaN(amount) ? -.1 : -amount, true)
        });
        ONECOLOR.installMethod("desaturate", function(amount) {
            return this.saturation(isNaN(amount) ? -.1 : -amount, true)
        });

        function gs() {
            var rgb = this.rgb(),
                val = rgb._red * .3 + rgb._green * .59 + rgb._blue * .11;
        ...
        ONECOLOR.installMethod("mix", function(otherColor, weight) {
            ...
            weight = 1 - (isNaN(weight) ? .5 : weight);
            var w = weight * 2 - 1,
